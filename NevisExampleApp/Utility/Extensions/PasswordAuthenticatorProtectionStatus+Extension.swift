		...
		switch self {
			...
			case let .LastAttemptFailed(remainingTries, coolDown):
				...
				switch (remainingTries, coolDown) {
					...
					case (1, 1...):
						localizedDescription = L10n.Credential.Password.ProtectionStatus.lastRetryWithCoolDown(
							remainingTries,
							String(format: "%.0f", Double(coolDown))
						)
					case (2..., 1...):
						localizedDescription = L10n.Credential.Password.ProtectionStatus.retriesWithCoolDown(
							remainingTries,
							String(format: "%.0f", Double(coolDown))
						)
					default:
